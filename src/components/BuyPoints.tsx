...
const PURCHASE_TIERS: Tier[] = [
  { points: 100, price: 10 },
  { points: 500, price: 50, bonus: 100 },
  { points: 1000, price: 100, bonus: 300 },
];

export default function BuyPoints({ onBack }: { onBack: () => void }) {
  const { userInfo, purchasePoints, addOrder } = useAuth();
  const [selectedTier, setSelectedTier] = useState<Tier | null>(PURCHASE_TIERS[0]);
  ...
  const currentPoints = selectedTier?.points || 0;
  const currentPrice = selectedTier?.price || 0;
  const currentBonus = selectedTier?.bonus || 0;
            ...
            {PURCHASE_TIERS.map((tier, i) => (
              <div 
                ...
                className={`relative p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-1 cursor-pointer ${
                  selectedTier === tier ? 'border-donghai bg-donghai/5' : 'border-white bg-white'
                }`}
              >
                <div className={`text-sm font-bold ${selectedTier === tier ? 'text-donghai' : 'text-gray-800'}`}>
                  {tier.points} 积分
                </div>
                <div className="text-[10px] text-gray-400">¥{tier.price}</div>
                {tier.bonus && (
                  <div className="text-[9px] text-orange-500 font-bold mt-0.5">送{tier.bonus}积分</div>
                )}
              </div>
            ))}
          </div>
